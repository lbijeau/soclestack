...
import { hasOrgRole, generateInviteToken, createInviteExpiry } from '@/lib/organization'
...
export async function GET() {
  try {
    ...
    if (!hasOrgRole(user.organizationRole, 'ADMIN')) {
      return NextResponse.json(
        { error: { type: 'AUTHORIZATION_ERROR', message: 'You do not have permission to view invites' } as AuthError },
        { status: 403 }
      )
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hasOrgRole(user.organizationRole, 'ADMIN')) {
      return NextResponse.json(
        { error: { type: 'AUTHORIZATION_ERROR', message: 'You do not have permission to invite members' } as AuthError },
        { status: 403 }
      )
    }
    ...
    if (hasOrgRole(role, user.organizationRole)) {
      return NextResponse.json(
        { error: { type: 'AUTHORIZATION_ERROR', message: 'You cannot invite someone with your role or higher' } as AuthError },
        { status: 403 }
      )
    }
    ...
    if (existingMember) {
      return NextResponse.json(
        { error: { type: 'VALIDATION_ERROR', message: 'This user is already a member of the organization' } as AuthError },
        { status: 400 }
      )
    }
    ...
    if (existingInvite) {
      return NextResponse.json(
        { error: { type: 'VALIDATION_ERROR', message: 'An invite has already been sent to this email' } as AuthError },
        { status: 400 }
      )
    }
