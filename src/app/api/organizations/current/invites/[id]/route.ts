...
export async function DELETE(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!hasOrgRole(user.organizationRole, 'ADMIN')) {
      return NextResponse.json(
        { error: { type: 'AUTHORIZATION_ERROR', message: 'You do not have permission to cancel invites' } as AuthError },
        { status: 403 }
      )
    }
    ...
    if (!invite || invite.organizationId !== user.organizationId) {
      return NextResponse.json(
        { error: { type: 'NOT_FOUND', message: 'Invite not found' } as AuthError },
        { status: 404 }
      )
    }
