...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!validationResult.success) {
      return NextResponse.json(
        { error: { type: 'VALIDATION_ERROR', message: 'Invalid input', details: validationResult.error.flatten().fieldErrors } },
        { status: 400 }
      )
    }
    ...
    if (user.role === 'ADMIN') {
      return NextResponse.json(
        { error: { type: 'FORBIDDEN', message: 'System administrators cannot delete their own account. Contact another admin.' } },
        { status: 403 }
      )
    }
    ...
    if (user.organizationId && user.organizationRole === 'OWNER') {
      return NextResponse.json(
        { error: { type: 'FORBIDDEN', message: 'You must transfer organization ownership before deleting your account.' } },
        { status: 403 }
      )
    }
    ...
    if (!user.password) {
      return NextResponse.json(
        { error: { type: 'BAD_REQUEST', message: 'OAuth-only accounts cannot be deleted this way. Please contact support.' } },
        { status: 400 }
      )
    }
